...
class DifferenceGenerator(BaseFeatureGenerator):
    """
    Generates difference features (e.g., current value - previous value).
    """
    ...
    def inspect(self, df: pd.DataFrame) -> dict:
        ...
        self.log_info(f"Will generate {expected_features} difference features.")
    ...
    def execute(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in self.target_columns:
            for period in self.periods:
                ...
                df_new[new_col_name] = df_new[col].diff(periods=period)
                
                self._create_metadata(
                    new_feature_name=new_col_name,
                    source_cols=[col],
                    formula=f"diff({period})"
                )
